...
#define NSec_Since_1582        0x01B21DD213814000ull
...
#pragma warning(pop)    /* C4996 */
	...
	struct timeval t;
    gettimeofday(&t, NULL);
    srand((uint32) ((((uint32)t.tv_sec&0xFFFF)+(uint32)t.tv_usec)^(uint32)t.tv_usec));
#endif
    return rand();
};
...
#pragma warning(pop)    /* C4996 */
    ...
    timeTmp = ((uint64) tv.time) * USec_Per_Sec + ((uint64) tv.millitm*USec_Per_MSec);
#else
	struct timeval t;
    gettimeofday(&t, NULL);
	timeTmp = ((uint64) t.tv_sec) * USec_Per_Sec + ((uint64) t.tv_usec*USec_Per_MSec);
#endif	
	
	
    timeTmp = timeTmp * 10 ;
    timeTmp += NSec_Since_1582;
    ...
    assert(u);
    u->data1 = (dword_t) timeTmp;
    u->data2 = (word_t) ((timeTmp >> 32) & 0xffff);
    u->data3 = (word_t) (((timeTmp >> 48) & 0x0ffff) | 0x1000);
...
char *uuid_to_string(const ios_uuid_t*  u)
{
...
void  uuid_create_external(const char *external, ios_uuid_t* u)
{
    MD5_CTX md5;
 ...
 **/
void uuid_to_timestamp(const ios_uuid_t* u, timestamp_t* t)
{
    uint64   timeTmp, t2, t3;
    struct  tm*  p;
    assert(u);
    ...
    timeTmp = u->data1 + (t2<<32) + ((t3&0x0fff)<<48);        /* 100ns */
    timeTmp -= NSec_Since_1582;

    t->tm_fraction = (long)(timeTmp%NSec100_Per_Sec);
    
    timeTmp /= 10;
    timeTmp /= USec_Per_Sec; 
    
#pragma warning(push)    /* C4996 */
#pragma warning( disable : 4996 )
#ifdef WIN32
    p = _localtime64(&timeTmp);
#else
	time_t tt = timeTmp;
	p=localtime(&tt);
#endif
...
*/
int uuid_compare(const ios_uuid_t *u1, const ios_uuid_t *u2)
{
    int i;
...
*/
int uuid_compare_time(const ios_uuid_t *u1, const ios_uuid_t *u2)
{    
#define CHECK_COMP(f1, f2)  if ((f1) != (f2)) return ((f1) < (f2) ? -1 : 1);
