...
public:
	...
	{
		if( 0 > QueryPerformanceData(pPerfData, dwObjectIndex, dwCounterIndex))
			return -1;
		...
		PPERF_OBJECT_TYPE pPerfObjFirst = pPerfObj;
	...
	T GetCounterValueForProcessID(PERF_DATA_BLOCK **pPerfData, 
			DWORD dwObjectIndex, DWORD dwCounterIndex, DWORD dwProcessID)
	{
		if( 0 > QueryPerformanceData(pPerfData, dwObjectIndex, dwCounterIndex) )
			return -1;
		...
		for( DWORD i=0; i < (*pPerfData)->NumObjectTypes; i++ )
		{
			...
			if (pPerfObj->ObjectNameTitleIndex == dwObjectIndex)
			{
				lnValue = GetCounterValueForProcessID(pPerfObj, dwCounterIndex,
														dwProcessID);
				break;
			}
...
protected:
	...
	//
	//
	int QueryPerformanceData(PERF_DATA_BLOCK **pPerfData, DWORD dwObjectIndex,
								DWORD dwCounterIndex)
	{
		...
		while(1)
		{
			lRes = RegQueryValueEx( HKEY_PERFORMANCE_DATA,
				keyName,
				NULL,
				NULL,
				Buffer,
			...
			if( lRes == ERROR_MORE_DATA )
			{
				...
				if( NULL == Buffer.Realloc(BufferSize))
					return -1;
			}
	...
	//
	T GetCounterValue(PPERF_OBJECT_TYPE pPerfObj, DWORD dwCounterIndex, 
						LPCTSTR pInstanceName)
	{
		...
		PPERF_COUNTER_DEFINITION pPerfInst2 = pPerfCntr;
		...
		{
			pCounterBlock = (PPERF_COUNTER_BLOCK) ((LPBYTE) pPerfObj + 
		...
		else
		{
			...
			for( int k=0; k < pPerfObj->NumInstances; k++ )
			{
				...
				{
					pCounterBlock = (PPERF_COUNTER_BLOCK) ((LPBYTE) pPerfInst + 
	...
	T GetCounterValueForProcessID(PPERF_OBJECT_TYPE pPerfObj, 
							DWORD dwCounterIndex, DWORD dwProcessID)
	{
		...
		else
		{
			...
			for( int k=0; k < pPerfObj->NumInstances; k++ )
			{
				...
				{
					DWORD processID  = 0;
					processID = static_cast<DWORD>(*(T*)((LPBYTE) pCounterBlock + 
									pProcIDPerfCntr->CounterOffset));
