// GMMarkupSTL.h: interface for the GMMarkupSTL class.
//
// Markup Release 8.2
// Copyright (C) 1999-2006 First Objective Software, Inc. All rights reserved
// Go to www.firstobject.com for the latest GMMarkupSTL and EDOM documentation
...
{
public:
	GMMarkupSTL() { SetDoc( NULL ); InitDocFlags(); };
	GMMarkupSTL( const char* szDoc ) { SetDoc( szDoc ); InitDocFlags(); };
	GMMarkupSTL( int nFlags ) { SetDoc( NULL ); m_nFlags = nFlags; };
	GMMarkupSTL( const GMMarkupSTL& markup ) { *this = markup; };
	void operator=( const GMMarkupSTL& markup );
	~GMMarkupSTL() {};
